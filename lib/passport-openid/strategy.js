...
function Strategy(options, verify) {
  ...
  this.attributeEx = options.attributeEx;
  ...
  if (options.profile) {
    var ax = new openid.AttributeExchange(this.attributeEx);
    extensions.push(ax);
  }
...
Strategy.prototype._parseProfileExt = function(params) {
  ...
  // extract attributeExtension data and add them to profile
  for (key in this.attributeEx) {
    if (!params[key]) {
